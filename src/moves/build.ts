...
function buildFromCommon(
  ...
  const newGameState = G.withPlayerAndGameStateSaved(ctx).withCardToBeBuilt(
    selectedCard,
    EnergyTypeDictionary.fromTypeAndAmount(
      selectedCard?.color,
      selectedCard?.cost
    )
  );
...
function buildFromArchive(
  ...
  const selectedCard: CardInfo | null = playerState.findCardInTheArchive(
    cardId
  );
  if (!selectedCard) return INVALID_MOVE;
...
function buildFromResearched(
  G: GameState,
  ctx: GameContext,
  cardId: number
): GameState | string {
  const playerState: PlayerState = G.players[ctx.currentPlayer];
  const selectedCard: CardInfo | null = playerState.findCardInTheResearched(
    cardId
  );
  if (!selectedCard) return INVALID_MOVE;

  const withCardsMoved = G.moveCard(
    From.PlayerResearched(ctx.currentPlayer, cardId),
    To.CardToBuild()
  ).moveCard(From.PlayerResearched(ctx.currentPlayer), To.BottomOfPile());
  ...
  const newGameState = withCardsMoved.withCardToBeBuilt(
    ...
    EnergyTypeDictionary.fromTypeAndAmount(
      withCardsMoved.cardToBeBuilt.color,
      withCardsMoved.cardToBeBuilt?.cost
    )
  );
