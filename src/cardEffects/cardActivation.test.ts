...
test.each`
  ...
  ${TriggerType.Build}   | ${TestCard(11, 2)}  | ${false}           | ${"Does not change the status on build picker if the card does not fit the condition"}
  ...
  ${TriggerType.Archive} | ${TestCard(11, 2)}  | ${false}           | ${"Does not change the status on archive trigger if the card does not fit the condition"}
